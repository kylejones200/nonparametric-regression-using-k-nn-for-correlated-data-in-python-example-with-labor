...
def main():
    ...
    plot_time_series(
        data, config["data"]["target_column"], output_dir / "raw_temperatures.png"
    )

    data = create_lag_features(
        data, config["features"]["max_lag"], config["data"]["target_column"]
    )
    ...
    y_pred_kernel = apply_kernel_smoothing(
        data, config["data"]["target_column"], config["model"]["kernel"]["bandwidth"]
    )
