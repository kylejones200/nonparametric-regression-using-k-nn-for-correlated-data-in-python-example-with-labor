...
def create_lag_features(
    data: pd.DataFrame, max_lag: int = 7, target_col: str = "Temp"
) -> pd.DataFrame:
...
def apply_kernel_smoothing(
    data: pd.DataFrame, target_col: str = "Temp", bw: str = "cv_ls"
) -> np.ndarray:
    """Apply kernel regression smoothing."""
    data_reset = data.reset_index()
    data_reset["Time"] = np.arange(len(data_reset))
    kr = KernelReg(
        endog=[data_reset[target_col]], exog=[data_reset["Time"]], var_type="c", bw=bw
    )
...
def plot_time_series(
    data: pd.DataFrame, target_col: str, output_path: Path, plot: bool = False
):
...
def plot_knn_predictions(
    y_test: pd.Series, y_pred: np.ndarray, output_path: Path, plot: bool = False
):
...
def plot_kernel_smoothing(
    ...
    if plot:
        ...
        ax.plot(
            data_reset["Date"], y_pred, label="Smoothed", color="#D4A574", linewidth=1.2
        )
        ax.set_xlabel("Date")
        ax.set_ylabel("Temperature (°C)")
        ax.legend(loc="best")
